...
//ajouter un collegue
router.route('/add/collegue/:id?').post((req, res) =>{
...
router.route('/get/collegue/:id?').get((req, res) => {
    ...
    const id_col=req.body.id_collegue
    
     db('users').whereIn
     ('id_user',db('collegue').where({id_user:id_e,confirm:true}).select('id_collegue'))
    .select('nom')
        .then((rows) => {
            res.json(rows)
        })
...
router.route('/delete/collegue/:id?').delete((req,res)=>{
    ...
    db('collegue').where({id_collegue:id,id_user:data }).delete('*')
    .then((rows) => {
        res.json(rows)
    })
    .catch((err) => {
...
router.route('/update/collegue/:id?').put((req,res)=>{
    ...
    data= req.body.id_user;
    db('collegue').where({id_collegue:id_ens,id_user:data }).update({
        confirm:true
    })
    .then((rows) => {
        res.json(rows)
